...
class CNN:
    ...
    def generate_model(self):
        ...
        # LAYER 1
        model.add(tf.keras.layers.Conv2D(32, (5, 5), activation='relu', input_shape=(256, 256, 3)))  # Shape: 256x256x3
        ...
        # LAYER 2
        model.add(tf.keras.layers.Conv2D(64, (5, 5), activation='relu', input_shape=(256, 256, 3)))
        model.add(BatchNormalization())
        model.add(tf.keras.layers.MaxPooling2D(2, 2))
        if dropout:
            model.add(tf.keras.layers.Dropout(0.3))
        ...
        # LAYER 3
        model.add(tf.keras.layers.Conv2D(128, (5, 5), activation='relu', input_shape=(256, 256, 3)))
        model.add(BatchNormalization())
        model.add(tf.keras.layers.MaxPooling2D(2, 2))
        if dropout:
            model.add(tf.keras.layers.Dropout(0.3))
        ...
        # LAYER 4
        model.add(tf.keras.layers.Conv2D(256, (5, 5), activation='relu', input_shape=(256, 256, 3)))
        model.add(BatchNormalization())
        model.add(tf.keras.layers.MaxPooling2D(2, 2))
        if dropout:
            model.add(tf.keras.layers.Dropout(0.3))
