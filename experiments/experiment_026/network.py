...
class CNN:
    ...
    def generate_model(self):
      ...
      # LAYER 1
      model.add(tf.keras.layers.Conv2D(32, (3, 3), activation='relu', padding='same',kernel_initializer="he_normal", input_shape=(256, 256, 3)))  # Shape: 256x256x3
      if batch_normalization:
        model.add(BatchNormalization())
      model.add(tf.keras.layers.MaxPooling2D(2, 2))
      if complexity=='high':
        model.add(tf.keras.layers.Conv2D(32, (3, 3), activation='relu',padding='same', input_shape=(256, 256, 3)))  # Shape: 256x256x3
        if batch_normalization:
          model.add(BatchNormalization())
        model.add(tf.keras.layers.MaxPooling2D(2, 2))
      if dropout:
